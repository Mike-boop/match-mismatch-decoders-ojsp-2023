...
if __name__ == '__main__':
    ...
    for split in ['test', 'heldout']:
        ...
        for sub in range(1, 86):
            
            
            results_dict[f'sub-{sub:03d}'] = {
                'envelope': {'predicted_labels':[], 'targets':[]},
                'ffr': {'predicted_labels':[], 'targets':[]},
                }
            if split == 'heldout':
                results_dict[f'sub-{sub:03d}'].update({
                'composite': {'predicted_labels':[], 'targets':[]},
                })


            averaged_env_outputs_files = glob.glob(
                os.path.join(averaged_env_outputs_dir, f'env_-_avg-{n_avg:03d}_-_{split}_-_sub-{sub:03d}*outputs.npz')
                )
            ...
            for i in range(len(averaged_env_outputs_files)):
                ...
                results_dict[f'sub-{sub:03d}']['envelope']['predicted_labels'].append(env_predictions[:max_len]>0)
                results_dict[f'sub-{sub:03d}']['envelope']['targets'].append(env_data['targets'])
                results_dict[f'sub-{sub:03d}']['ffr']['predicted_labels'].append(ffr_predictions[:max_len]>0)
                results_dict[f'sub-{sub:03d}']['ffr']['targets'].append(env_data['targets'])

                if split == 'heldout':
                    lda_features = np.vstack([env_predictions[:max_len], ffr_predictions[:max_len]]).T
                    results_dict[f'sub-{sub:03d}']['composite']['predicted_labels'].append(clf.predict(lda_features).squeeze())
                    results_dict[f'sub-{sub:03d}']['composite']['targets'].append(env_data['targets'])

            
            for decoder_key in results_dict[f'sub-{sub:03d}']:
                
                hits = np.concatenate(results_dict[f'sub-{sub:03d}'][decoder_key]['predicted_labels']) == \
                    np.concatenate(results_dict[f'sub-{sub:03d}'][decoder_key]['targets'])
                
                n_observations = len(hits)
                accuracy = hits.sum()/len(hits)

                results_dict[f'sub-{sub:03d}'][decoder_key] = {
                    'n_obs': n_observations,
                    'acc': accuracy
                }

            
        with open(os.path.join(results_savedir, f'sparrkulee_{split}_results.json')) as f:
